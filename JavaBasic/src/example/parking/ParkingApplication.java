...
//		if(type != null && type != "경차" || type != "중대형" || type != "SUV") {
		...
		for (int layer = 0; layer < registerNumbers.length; layer++) {
			for(int space = 0; space < registerNumbers[layer].length; space++) {
				if(registerNumber.equals(registerNumbers[layer][space])) {
					System.out.println(registerNumber + "의 차량은 " + layer + "층 " + space + "번에 있습니다.");
					found = true;
					break;
				}
			}
		...
		if(leaveTime < 0 || leaveTime > 24) {
			System.out.println("출차시간을 정확히 입력하세요.");
			return;
		}
		...
		// (출차시간 - 입차 시간) * 차종 금액
		int typeAmount = types[foundLayer][foundSpace].equals("경차") ? 1000 :
